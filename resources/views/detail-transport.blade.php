                ...
                <div>
                    <div class="bg-white rounded-3xl shadow-lg p-8">
                        <h1 class="text-3xl font-bold text-gray-800 mb-4">
                            {{ $transport->serviceProvider->providerName }}</h1>
                        ...
                        <div class="bg-gray-50 border-2 border-gray-100 rounded-2xl p-6 mb-8 text-center">
                            <p class="text-3xl font-bold text-gray-900">Rp
                                {{ number_format($transport->averagePrice, 0, ',', '.') }} <span
                        ...
                        <div class="space-y-4 mb-8 text-sm">
                                ...
                                <div class="text-right">
                                    @if ($transport->facilities)
                                        @foreach ($transport->facilities as $fac)
                                            <span
                                                class="inline-block bg-green-50 text-[#2CB38B] px-2 py-1 rounded text-xs font-bold mb-1">{{ $fac }}</span>
                                        @endforeach
                                    @else
                                        -
                                    @endif
                            ...
                            <div class="flex justify-between py-3">
                                ...
                                <a href="{{ $transport->bookingLink }}" target="_blank"
                                    class="text-[#2CB38B] hover:underline font-semibold truncate max-w-[200px]">
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center space-x-4 bg-gray-100 rounded-full px-6 py-3">
                                <button @click="if(quantity > 1) quantity--"
                                    class="w-8 h-8 flex items-center justify-center hover:bg-gray-200 rounded-full transition text-gray-600 font-bold">-</button>
                            ...
                            <button @click="showModal = true"
                                class="flex-1 bg-[#2CB38B] hover:bg-[#249d78] text-white font-bold py-4 px-8 rounded-full text-lg transition shadow-lg hover:shadow-xl">
                                Tambah ke Rencana
                            </button>
                        </div>
        ...
        <div x-show="showModal"
            ...
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 relative m-4">
                ...
                <form action="{{ route('plan.add-item', $travelPlan->planID) }}" method="POST">
                    ...
                    <button type="submit"
                        class="w-full bg-[#2CB38B] hover:bg-[#249d78] text-white font-bold py-4 rounded-xl shadow-lg transition">
                        Simpan
                    </button>
                </form>
