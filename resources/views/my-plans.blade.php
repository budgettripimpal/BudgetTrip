        ...
        <div class="container mx-auto px-6 max-w-7xl">
            ...
            <div class="flex justify-between items-end mb-8">
                ...
                <a href="{{ route('travel-plan.create') }}"
                    class="bg-[#2CB38B] hover:bg-[#249d78] text-white px-6 py-3 rounded-xl font-bold shadow-lg flex items-center gap-2 transition transform hover:-translate-y-1">
            ...
            @if ($plans->isEmpty())
                <div class="bg-white rounded-3xl p-16 text-center shadow-sm border border-gray-100">
                    <div class="w-24 h-24 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-6">
                        <svg class="w-12 h-12 text-[#2CB38B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <p class="text-gray-500 mb-8 max-w-md mx-auto">Ayo mulai petualanganmu sekarang! Buat rencana
                        perjalanan pertamamu dengan mudah dan hemat.</p>
                    <a href="{{ route('travel-plan.create') }}" class="text-[#2CB38B] font-bold hover:underline">Mulai
                        Sekarang &rarr;</a>
                </div>
            @else
                    ...
                    @foreach ($plans as $plan)
                        <div
                            ...
                            <div class="h-32 bg-gradient-to-r from-[#2CB38B] to-teal-600 relative p-6">
                                <div
                                    class="absolute top-4 right-4 bg-white/20 backdrop-blur-sm px-3 py-1 rounded-lg text-white text-xs font-bold border border-white/30">
                                    {{ $plan->startDate->format('d M') }} - {{ $plan->endDate->format('d M Y') }}
                                </div>
                                <h3 class="text-white font-bold text-xl mt-8 truncate drop-shadow-md">
                                    {{ $plan->planName }}</h3>
                            </div>
